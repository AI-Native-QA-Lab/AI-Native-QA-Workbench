...
-   v0.2 Evidence Foundation 已在当前 checkout 完成本地实现；本轮不创建新的 tag、GitHub Release、远端 Milestone 或 Issue。
...
## v0.2 Evidence Foundation Implementation Gate（2026-09-22）

-   本地实现边界包括 Evidence Domain/Contract、Evidence YAML manifest、
    JUnit/Playwright/Pytest Adapter、artifact checksum/size/orphan 校验、
    Import/Verify Application Service，以及 `qaw evidence import/verify`。
-   Evidence 默认保持 `unverified`；CLI 不计算 Quality Score，也不能通过参数伪造
    trusted 状态。Project Quality Source of Truth 仍是 `.ai-qa/` 文件，SQLite
    仍只承载 runtime/operational state。
-   本地验证完成后，结果记录见
    [`docs/superpowers/records/2026-09-22-v0-2-evidence-verification.md`](../superpowers/records/2026-09-22-v0-2-evidence-verification.md)。
